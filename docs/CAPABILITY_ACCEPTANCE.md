...
| C17 Classification editing / undo | Implemented, limited validation (d76392a): section editor with undo/redo and verified export; real-data acceptance still pending | Viewer and GUI review | Need stable point identities, durable edit history, selection tools, undo/redo and round-trip verification. Not just drawing a selection rectangle. |
...
| C20 Release, license and reproducibility | Release gap. `pyproject.toml`, `packaging/` | `tests/test_packaging_gate.py`, current branch history | Dependencies have broad minimums. License CLOSED 2026-09-17 (PRs #1/#3): Apache-2.0, copyright Mapworks, LLC, LICENSE at the root and declared in pyproject. Distribution dependencies still to be reviewed before claiming an unrestricted open-source release. Binary/source provenance and repeatable build remain incomplete. |
